---
title: "SpringAI中ChatModel和ChatClient有啥区别？"
...
+ `**ChatModel**` 是**引擎**：负责直接与 AI 模型提供商（如 OpenAI、Azure、本地模型）通信，处理底层的请求和响应。
+ `**ChatClient**` 是**驾驶舱/控制台**：建立在 `ChatModel` 之上，为开发者提供流畅的 API（Fluent API），用于编排对话流、管理上下文、调用工具（Function Calling）以及处理响应转换。
